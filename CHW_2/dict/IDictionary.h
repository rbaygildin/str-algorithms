...
pair<vector<string>, vector<string>> IDictionary::findOccurrences(istream &input) {
    ...
    while(input >> temp_){
         ...
         */
        if(count_if(temp_.begin(), temp_.end(), [](char c){
            return !isalpha(c);
        }) == temp_.length())
            continue;
        temp_.erase(std::remove_if(temp_.begin(), temp_.end(), [](char c){
            return !isalpha(c);
        }), temp_.end());
        if(isIncluded(temp_))
            occurrences_.push_back(temp_);
        else
            notIncluded_.push_back(temp_);
    }
